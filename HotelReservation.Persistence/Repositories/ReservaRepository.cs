...
namespace HotelReservation.Persistence.Repositories
{
    public class ReservaRepository : IReservaRepository
    {
        ...
        VALUES (
            ...
            // 1. Obtener el PrecioPorNoche de la Categor�a de la Habitaci�n
            var precioQuery = @"
        ...
        WHERE h.Id = @HabitacionId";
            ...
            {
                // Manejar el caso de que la habitaci�n o su categor�a no exista.
                throw new InvalidOperationException($"No se pudo encontrar el precio por noche para la habitaci�n {reserva.HabitacionId}.");
            }

            // ARREGLO 1: Usar '!' para indicar al compilador que ya se verific� la nulidad.
            decimal precioPorNoche = (decimal)precioResult!;

            // 2. Calcular la diferencia de d�as (FechaFin - FechaInicio)
            // Se asume que FechaInicio y FechaFin est�n en la entidad Reserva.
            int dias = (int)(reserva.FechaFin - reserva.FechaInicio).TotalDays;
            if (dias <= 0)
            {
                // Si son menos de un d�a, podr�a ser 1 d�a si la pol�tica lo exige, o lanzar un error.
                // Usaremos Math.Max(1, dias) si la pol�tica es cobrar al menos un d�a.
                // Para este ejemplo, si es 0 o menos, lanzamos una excepci�n simple.
            ...
            // Asignar el total calculado a la entidad antes de la inserci�n
            reserva.Total = totalCalculado;

            // A�adir todos los par�metros al comando INSERT
            ...
            cmd.Parameters.AddWithValue("@TotalCalculado", totalCalculado); // Nuevo par�metro

            // Ejecutar el INSERT y obtener el ID
            // ARREGLO 2: Usar '!' para suprimir el warning, ya que 'OUTPUT INSERTED.Id'
            // garantiza que se devolver� un GUID si el INSERT tiene �xito.
            ...
            {
                throw new InvalidOperationException("El Id insertado tiene un formato inv�lido.", ex);
            }
        }
